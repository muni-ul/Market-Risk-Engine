    ...
    def apply_trade(self, trade_row) -> None:
        ...
        if fill_price < 0:
            raise PortfolioError(
                f"cannot apply trade with negative fill_price. Received {fill_price}."
            )
        if commission < 0:
            raise PortfolioError(
                f"cannot apply trade with negative commission. Received {commission}."
            )
        ...
        if side == "BUY":
            ...
            if total_cost > self.cash:
                raise PortfolioError(
                    f"insufficient cash for BUY order. Required {total_cost:.2f}, "
                    f"available {self.cash:.2f}."
                )
            old_qty = position.quantity
            new_qty = old_qty + quantity
            position.average_cost = (
                (old_qty * position.average_cost) + (quantity * fill_price)
            ) / new_qty
            ...
            if not self.allow_short and quantity > position.quantity:
                raise PortfolioError(
                    f"cannot sell {quantity} {symbol} contracts when current "
                    f"position is {position.quantity}."
                )
            self.cash += notional - commission
            self.realized_pnl += (
                (fill_price - position.average_cost) * quantity * self.contract_multiplier
                - commission
            )
...
def build_portfolio_history(
    ...
    if missing:
        raise PortfolioError(
            f"pricing_history is missing required columns: {', '.join(sorted(missing))}."
        )
