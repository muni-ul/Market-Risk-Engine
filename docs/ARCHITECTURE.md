...
Config validation happens before simulation work begins. Numeric, finite-value, integer, boolean, enum, and range checks raise field-specific `ConfigError` messages so user mistakes fail early and do not leak into pricing, portfolio, risk, or reporting code.
